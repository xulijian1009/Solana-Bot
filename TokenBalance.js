...
const OKX_TOKEN_BALANCE = process.env.OKX_TOKEN_BALANCE;  //  OKX TOKEN BALANCE
const OKX_TOKEN_BALANCE_API = process.env.OKX_TOKEN_BALANCE_API;  //  OKX TOKEN BALANCE API
...
// 调用OKX的API获取钱包指定代币数量
async function getTokenBalance(address,tokenAddresses) {
    try {
        ...
        const params = {
            ...
            tokenAddresses: tokenAddresses.map(address => ({
                chainIndex: '501',  // 主网 chainIndex，可以根据需要调整
                tokenAddress: address
            })),
            filter: '1'  // 过滤风险空投币，'0' 为过滤，'1' 为不过滤，默认过滤
        };

        // 构建 URL
        const requestPath = OKX_TOKEN_BALANCE_API;
        ...
        console.error('查询代币余额发生异常:', e.message);
    }
}
