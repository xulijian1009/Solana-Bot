...
async function getBalance() {
    try {
        const balance = await getTokenBalance(keypair.publicKey.toBase58(),['']);
...
// 调用OKX的DEX交换API进行代币购买
async function purchaseTokens(tokenContractAddress, amountInSol, slippage) {
    try {
        // 获取当前账户余额
        const solBalance = await getBalance() / LAMPORTS_PER_SOL;
        if (solBalance < amountInSol) {
            console.log(`余额不足，当前SOL余额为 ${solBalance} SOL`);
            return false;
        }
        ...
        const params = {
            chainId: 501,  // 主网
            amount: String(Math.floor(amountInSol * LAMPORTS_PER_SOL)),  // 以 lamports 为单位，转换 SOL
            toTokenAddress: tokenContractAddress,  // 目标代币合约地址
            fromTokenAddress: OKX_SOL_ADDRESS,  // SOL 地址
            slippage,  // 滑点
        ...
        if (responseData.code === '0') {
            ...
            // 现在调用钱包API进行交易广播
            const buySuccess = await sendTransaction(tx.data);
            return buySuccess;
        } else {
        ...
        console.error('购买代币时发生异常:', e.message);
        return false;
    }
}
...
// 外部调用时传递tokenContractAddress, amountInSol, slippage
module.exports = {purchaseTokens};
